...
void loop() {
  ...
  int lightpattern;
  int X = analogRead(xpin)-430; // The value subtracted is the raw sensor value removed to give us 0
  int Y = analogRead(ypin)-365;
  int Z = analogRead(zpin)-357;
  int XYZavg;
  int sensVal;
...
// Light pattern 1 (rest/attract mode)
  ...
  if ((lightpattern >= 0) && (lightpattern <= 25)) {
  sinelon();
  }

// Light pattern 2 (detects light movement)

  else if ((lightpattern >= 26) && (lightpattern <= 50)) {
  juggle();
  } 

// Light pattern 3 (detects heavier movement)

  else if ((lightpattern >= 51) && (lightpattern <= 100)) {
  bpm();
  }

// Light pattern 4 (hardest hit)

   else if ((lightpattern >= 101) && (lightpattern <= 255)) {
  rainbowWithGlitter();
  }
